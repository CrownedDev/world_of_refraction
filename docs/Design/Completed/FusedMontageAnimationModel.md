...
### D9 — Tier-gap DAMAGE scaling (separate from wear)

**The damage mechanic stands alone — wear is a SEPARATE, pre-existing system; do not conflate.**

An action's tier is compared to the tier of the **channel it's used through**, scaling its damage:
- **Spell** → the **crystal** it's cast through
- **Attack / Ability** → the **weapon**
...
multiplicatively at damage assembly (the `× (1 − RequirementPenalty)` precedent,
`AbilityData.cpp:20`, attack `:1017`, spell `:719`). Likely lives beside `GetTierGap`
(`TierHelpers`, ItemTier.h) — which today has only 3 callers, all in `BreakCalculator`, so a damage
sibling slots in cleanly.

**Prerequisites (from the gap survey):**
1. **Channel-tier resolution** — for abilities/attacks, `Weapon->Tier` is accessible at cast time
   (read 4× already). For spells, the crystal tier is resolved ONLY on the infused path today —
   uninfused casts never look up their crystal. A **per-cast channel-tier resolution** must be
   built (the lookup machinery exists: `FindAttachedItemByHolder` + `Crystal.Id.Tier`; it just
   isn't invoked uninfused). The 4× duplicated channel-resolution block in `ApplyCommitCosts`
   (:4333/:4390/:4469/:4536) is a de-dup opportunity — one resolver feeds both this and wear.
2. **Ability tier convention ruling** — the code already disagrees: wear path uses `Weapon->Tier`
   as the ability's action tier, BD-break path uses `AbilityData->Tier`. The damage mechanic needs
   **action tier = the SKILL's own tier** (`AbilityData->Tier`/`Spell->Tier`/`Attack` tier), gapped
   against the **channel** (weapon/crystal). Rule this so a third convention isn't added.
3. **AI preview parity** — the AI previews damage via `UDamageCalculator` (`AIDecisionManager.cpp:
   714/:769`); the multiplier must be visible to the AI's damage input or enemy decisions use wrong
   numbers.

**NOTE on wear:** wear is its OWN system (action vs CRYSTAL, commit-time, luck-skippable). It is NOT
part of this damage mechanic. They both happen to use tier comparisons but are independent. (A
future deliberate coupling — bad fit = weaker hit AND more wear — is possible but NOT in scope here.)

**NEW design, not unification cleanup.** Build first (Crown's call) — foundational.
