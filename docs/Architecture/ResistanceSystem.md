...
- **Byte-identical for neutral matchups:** where a resolved Matched column / effect contributes `0` (a Resonator hit by any element, a Caster hit by a neutral element with `None` physical, any `None`-element / `None`-physical hit — `None` is the non-elemental sentinel, mapped to the zero column exactly as `Generic` was pre-migration), the Matched term is an exact `0.0f`.
