...
`UEquipmentDataBase` (weapon + ring) holds `BaseResistance` (authored) + `GeneratedResistance` (rolled), summed by `GetCombinedResistance()`, **copied into the inventory entry's `ResistanceBonus` at `CreateFromWeapon`/`CreateFromRing` time** — the per-instance copy is the runtime truth (so asset edits/rolls only reach **new** entries). The `Roll Resistance` `CallInEditor` button (via `IEquipmentGenerator::RollResistance` / `GetEditableResistance`) writes `GeneratedResistance`; `UEquipmentBonusGenerator::GenerateResistance(Tier)` is the gear-agnostic runtime loot path.

**Evolution** (`UEvolutionItemData`) carries the same `Base + Generated + GetCombinedResistance` trio, rolled via a **standalone `Roll Resistance` button** (evolution does **not** implement `IEquipmentGenerator` — it has no stat roll, so forcing the interface was avoided). Evolution resistance is **ALWAYS-ON while slotted** — read live from the crystal asset via `GetActivePrimaryEvolutionCrystal` (no per-instance entry), **deliberately decoupled** from evolution's infusion-scaled offensive `BaseStatBonus` (a separate path, untouched).

### Read — source #6

`ULoadoutComponent::GetActiveResistanceBonus(Actor)` aggregates the per-instance `ResistanceBonus` across equipped pieces (weapon + ring per class, same resolution as `GetActiveStatBonus`) **plus** the evolution crystal's `GetCombinedResistance()`. `GetTotalStatusResistance` reads it as **term #6** by viewing the aggregate as an `FResistanceRow` and calling the **same** `GetElementColumn`/`GetPhysicalColumn` helpers as the class profile — so the **BD→Darkness alias is identical by construction** (incoming `BrokenDarkness` reads the gear's Darkness column) and cannot drift. Additive, shares the single final clamp.
