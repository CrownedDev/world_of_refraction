...
4. **Editor validation (`IsDataValid`).** The base checks `Name` is non-empty
   and that `DefaultSpells.Num()` does not exceed `GetMaxSpells()`. Subclasses
   add their own validation (e.g. `UWeaponData` rejects
   `PhysicalDamageType::None`; `UAbilityData`, `USpellData`, and `URingData`
   each override `IsDataValid`).
...
- `EquipmentBonusGen` (free functions) — tier budgets (`GetSubstatBudget`,
  `GetPillarBudget`) for the roll buttons.
