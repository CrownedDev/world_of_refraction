...
6. **Target resistance reduction.** `Amount *= (1 − GetTotalStatusResistance(Target, Element, PhysicalType))`. The whole composition was extracted into `GetTotalStatusResistance` (the single source of truth, defender-side only): base `CalculateResistance()` + equipment `BonusResistance × RESISTANCE_PER_POINT` + attached `ResistanceStone` + `GetTotalElementResistance(Target, Element, PhysicalType)` (element **or** physical effects) + `ClassInnateResistanceTable::GetClassInnateResistance(...)` + `ModifyStatusResist/100`, single final clamp to `[RESISTANCE_MIN, RESISTANCE_MAX]` (`[−1, +1]`), post-clamp value returned. The base `CalculateResistance()` keeps its internal `[0, MAX]` pre-clamp (intentional — weaknesses always bite). Returns `0` when the target has no `CharacterData`, so `Amount` is unchanged. Offense-side amps (StatusMultiplier, BD stack) are applied earlier and are **not** part of this. See `ResistanceSystem.md`.
