...
6. **Target resistance reduction.** If `Target` has a `UCharacterDataComponent` + `CharacterData`: start with `CharacterData->CalculateResistance()`, add `BonusResistance × RESISTANCE_PER_POINT` from the target loadout, add the attached `ResistanceStone` percent, add `GetTotalElementResistance(Target, Element)`, add **`ClassInnateResistanceTable::GetClassInnateResistance(Target, Element, PhysicalType)`** — the class/innate-element resistance term (see `ResistanceSystem.md`) — add `ModifyStatusResist/100` from skill effects, clamp to `[RESISTANCE_MIN, RESISTANCE_MAX]` (`[−1, +1]` — a net weakness pushes `Resistance` negative and amplifies buildup, capped at `−1` = ×2), then `Amount *= (1 - Resistance)`. The class/innate term is one sign-preserving additive line and is exactly `0` for neutral/0-column matchups (byte-identical to pre-system behaviour).
