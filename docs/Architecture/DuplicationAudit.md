...
- **AI defense path does NOT re-implement player defense math** (the high-value catch — clean here). `UAIDecisionManager::ChooseDefenseType` (`AIDecisionManager.cpp:411`), `GetDefenseAccuracy(EAIDifficulty)` (`:482`), `CalculateDefenseReactionDelay` (`:499`) operate on **`EAIDifficulty`** (the AI's *skill* level) — a different concept from the attack's per-impact **`EDefenseDifficulty`**. The AI only *decides what to submit* via `SubmitDefenseInput`; the window/result are owned by `UDefenseSystem` (`MatchAndConsumeInput` / `CalculateDefenseResult`). No mirrored player calc. *(Deliberate asymmetry — excluded.)*
