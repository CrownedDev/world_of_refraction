...
- `bOverrideStatScaling` (`bool`) — the **hybrid stat toggle** (consumed; see Step 1). When true, scaling uses the OPPOSITE stat to `ActionType` (Raw↔Spell): a physical attack scales `SpellDamage` (a "fire punch"), a spell scales `RawDamage` (a "force slash"). **Stat-only** — element routing and the Spell-mode branches stay on `ActionType`. Default false = natural scaling. Sourced upstream by `ActionExecutor` (physical: the skill-root flag, attack-wide; spell: per-cast off `FSkillCastEntry`); the calculator only reads the resolved `Input` bool.
...
2. **Step 1 — Attacker damage multiplier.** A derived `ScalingType` selects the stat axis: it is `Input.ActionType`, unless `Input.bOverrideStatScaling` is set, which swaps it to the opposite axis (`Spell`↔`Ability`) — the **hybrid stat toggle** (`DamageCalculator.cpp:50`). `GetAttackerDamageMultiplier` returns the crystal-aware Spell or Raw damage multiplier (branched on `ScalingType`), and `ActionMods.ApplyTo` boosts the matching sub-stat (`SpellDamage` when `ScalingType == Spell`, else `RawDamage`). The toggle is **stat-only** — element interaction and the Spell-mode branches below still read `Input.ActionType`, not `ScalingType`. Equipment stat bonus is read directly from the attacker's `ULoadoutComponent` (`GetActiveStatBonus`) and folded into `AttackerMult` as a fractional multiplier (`BonusRawDamage * RAW_DAMAGE_PER_POINT` or `BonusSpellDamage * SPELL_DAMAGE_PER_POINT`). `RunningDamage *= AttackerMult`.
