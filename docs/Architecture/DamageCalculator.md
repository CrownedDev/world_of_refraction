...
- *`CalculateHealing(Healer, Target, BaseHealing)` — **removed** (`chore/modifyhealing-cleanup`, 2026-06). Dead chain: its only caller was the equally-dead `UActionExecutor::ApplyHealing` (zero callers, BP-confirmed clear), so the function and its lone `ModifyHealing` read died together. The healer-side `GetEffectiveSpellDamage`/`ModifyHealing` heal scale had no live path; recipient-side amplification now lives in `HealingReceivedBuff` ("Healing Boost"). The `ModifyHealing` enum value is kept but `UMETA(Hidden)` (append-only + EnumRedirects).*
...
Any combat caller invoking `CalculateDamage` / `CalculateAttackDamage` (e.g. action execution). These call sites are outside the two files reviewed and were not traced here. (`CalculateHealing` removed — see Status/healing helpers.)
...
| 2026-06-23 | **ModifyHealing dead-code removal** — deleted the dead healer-side heal chain: `UActionExecutor::ApplyHealing` (zero C++ callers, BP-confirmed clear) and `UDamageCalculator::CalculateHealing` (its only caller). The lone `ModifyHealing` read died with `CalculateHealing`. `GetEffectiveSpellDamage` / `GetSkillEffectManager` untouched (live elsewhere). The `ModifyHealing` enum value is **kept + `UMETA(Hidden)`** (append-only stamping + `DefaultEngine.ini` EnumRedirects); recipient-side healing amplification lives in `HealingReceivedBuff`. | chore/modifyhealing-cleanup |
