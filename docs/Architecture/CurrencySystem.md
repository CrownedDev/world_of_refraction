...
`UCurrencyComponent`, holding six currencies behind one unified Add / Spend /
...
**The six currencies** (`ECurrencyType`):

| Currency | Storage | Tags | Scope (today) |
|----------|---------|------|---------------|
| `Prismas` | `int32` | **Replicated only** (run-volatile, never banked) | per-character (run) |
...
| `Dust` (14 keys) | `FCurrencyArray` (FastArray) | `SaveGame` + `Replicated` | per-character |
| `Essence` (2 pools) | `FCurrencyArray` (FastArray) | `SaveGame` + `Replicated` | per-character |

- **Dust keys** (`EDustType`, 14): 10 element (`Fire`…`Generic`), 3 pillar (`Mind`/`Body`/`Spirit`), 1 `Ability`.
- **Essence pools** (`EEssencePool`, 2): `Weapon`, `Crystal`.
- The scalar currencies are plain `int32` with `ReplicatedUsing = OnRep_*`. Dust and Essence
  use a FastArray (`TMap<Enum,int32>` is not natively replicable — see `Resources_Design.md` §16.2).
...
- `bool CanAfford(...) const` / `int32 GetBalance(...) const` — const, ungated reads (0 for an absent Dust/Essence key).
- `SubKey` selects the Dust/Essence entry (cast `EDustType`/`EEssencePool`); ignored for the scalars.

**Typed convenience** (thin wrappers over the unified API): `AddDust`/`SpendDust`/`GetDust(EDustType)`,
`AddEssence`/`SpendEssence`/`GetEssence(EEssencePool)`.
...
PIE-correct). Persistent balances are `SaveGame`-tagged for a future save system; `Prismas` is
...
- **`FCurrencyEntry : FFastArraySerializerItem`** — one keyed balance (`uint8 Key`, `int32 Amount`).
  `Key` is a `uint8`-cast `EDustType`/`EEssencePool`, disambiguated by the owning array's `WalletType`.
...
- **`FCurrencyArray : FFastArraySerializer`** — `TArray<FCurrencyEntry> Items` + `NetDeltaSerialize`
  (`WithNetDeltaSerializer`). A **single reusable** struct backs **both** the Dust and Essence wallets;
  each is a distinct `UPROPERTY` member on the component. Non-replicated `OwnerComponent` + `WalletType`
...
`uint8`-cast Dust/Essence key, or `0` (ignored) for the scalars.

### Debug tooling

- **`UCurrencyComponentDebug`** (`UBlueprintFunctionLibrary`) — `static FString GetWalletString(const UCurrencyComponent*)`:
  a one-line dump. Scalar currencies are **always** shown (incl. zero); Dust/Essence list only
...
- **No reward/faucet sources.** Nothing awards currency in C++ yet (the post-defeat reward hook is
  greenfield — see `Resources_Design.md` BUILD-STATE NOTE). The Essence faucet
  (`AddEssence(EEssencePool::Crystal, …)` from a later deconstruct step) is public and ready but uncalled.
