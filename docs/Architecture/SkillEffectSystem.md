...
- `HealingReceivedBuff` — recipient-side healing amplifier and the **mirror of `HealBlock`**. Passive presence-read marker (no-op in `ApplyEffectLogic`), summed via `GetTotalStatModifier` and applied at the **same four heal sites `HealBlock` gates** — HoT `HealthRestore` / `RestoreHPPercent` ticks (bearer), drain-on-hit (`User`), lifesteal-on-hit (`Attacker`) — in the else branch *after* the gate (**gate-then-amplify**): `Amount × (1 + bonus% / STAT_PERCENT_DIVISOR)`. Distinct from the dead healer-side `ModifyHealing` ("Healing Up"). Revive (`ServerResurrect`) is unaffected. No granter authored yet.
...
| 2026-06-22 | **Effect feature arcs (back-fill)** — `Charges` (finite-fire governor, separate from duration; charged-dur-0 footgun guard at the build choke point); **Last Stand** death-intercept (`ConsumeLastStandCharge`, best-of-N) — renamed from `Revive`; three-axis lifecycle — duration / timing / fire-on-apply decoupled (`bPermanent` explicit, `bDelayFirstExecution`, **dur-0 = INSTANT** lane: runs `ApplyEffectLogic` once, never stored); `HealBlock` enforcement. | feature/instant-effects, feature/effect-strength-policy, feature/effect-expansion |
| 2026-06-22 | **Effect-build unification** — extracted `FActiveSkillEffect::BuildRuntimeFromPayload` single-payload core; `CreateAllFromSkillEffect` (gear — byte-identical), the spell cast path (`ActionExecutor::ApplySkillEffects`), and the defender-trigger path now all route through it; **deleted `CreateFromSpellEffect`** — its dur-0 clamp + silently-dropped `bPermanent`/`Charges`/`bDelayFirstExecution` are gone, so spells now honour those four fields. `EffectValue` preserved via post-build override (cast EffMagMult / defender pre-rounding); ProcessTiming type-detect is an opt-in core param (`bAutoDetectTimingByType`). | feature/effect-build-unification |
| 2026-06-23 | **`HealingReceivedBuff`** — recipient-side healing amplifier, the symmetric mirror of `HealBlock`. Appended enum value (DisplayName "Healing Boost", `IsBuff` → true); passive presence-read no-op in `ApplyEffectLogic`; amplifies at the same four heal sites `HealBlock` gates (HoT `HealthRestore`/`RestoreHPPercent` ticks, drain, lifesteal) in the else branch after the gate (gate-then-amplify), via `GetTotalStatModifier` / `STAT_PERCENT_DIVISOR`. Healer-side `ModifyHealing` and revive untouched; no granter authored yet. | feature/healing-received |
| 2026-06-23 | **`ModifyHealing` retired** — the dead healer-side heal chain (`UActionExecutor::ApplyHealing` → `UDamageCalculator::CalculateHealing` → the lone `ModifyHealing` read) was deleted. The `ModifyHealing` enum value is **kept + `UMETA(Hidden)`** (append-only stamping + `DefaultEngine.ini` EnumRedirects forbid deletion); its `IsBuff`/display-name/`ApplyEffectLogic`-no-op sites remain harmless (reference a still-present hidden value). Recipient-side amplification is `HealingReceivedBuff`. | chore/modifyhealing-cleanup |
