...
- **MaxPool** = the roll's point budget. Seeded at acquisition from the per-asset override (`Stat/ResistanceMaxPoolOverride` > 0) or the tier budget (`POOL_OVERRIDE_USE_TIER` = 0 sentinel; `SUBSTAT_BUDGET_*` / `RESISTANCE_BUDGET_*`).
